...
fastify.get('/youtube/embed', async (request, reply) => {
  const src = typeof request.query?.src === 'string' ? request.query.src : '';
  const embedUrl = youtubeEmbedUrl(src);

  if (!embedUrl) {
    return reply.status(400).type('text/plain').send('Invalid YouTube URL.');
  }
  ...
  return reply.type('text/html').send(youtubeEmbedPage(embedUrl));
});
...
fastify.post('/notion/pages', async (request) => {
  const body = (request.body ?? {}) as CreateNotionPageRequest;
...
fastify.post('/sync/push', async (request) => {
  const body = (request.body ?? {}) as SyncPageRequest;
...
fastify.post('/sync/project', async (request) => {
  const { project, selectedParentPageId } = (request.body ?? {}) as SyncProjectRequest;
...
fastify.post('/sync/validate', async (request) => {
  const { pages = [], projects = [] } = (request.body ?? {}) as SyncValidationRequest;
  const store = await requireConnectedStore(request);
...
fastify.post('/sync/reload', async (request) => {
  const { selectedParentPageId } = (request.body ?? {}) as SyncReloadRequest;
  const store = await requireConnectedStore(request);
...
fastify.post('/media/upload', async (request, reply) => {
  const { dataBase64, mimeType, filename } = (request.body ?? {}) as MediaUploadRequest;
...
fastify.post('/sync/pull', async (request) => {
  const { page } = (request.body ?? {}) as SyncPageRequest;
