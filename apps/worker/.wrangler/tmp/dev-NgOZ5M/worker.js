    ...
    const executeWithRetry = /* @__PURE__ */ __name(async () => {
      ...
      while (true) {
        ...
        if (shouldRetry(_this.method, res$1.status, attemptCount, _this.retryEnabled)) {
          ...
          const retryAfterHeader2 = (_res$headers$get = (_res$headers = res$1.headers) === null || _res$headers === void 0 ? void 0 : _res$headers.get("Retry-After")) !== null && _res$headers$get !== void 0 ? _res$headers$get : null;
          const delay = retryAfterHeader2 !== null ? Math.max(0, parseInt(retryAfterHeader2, 10) || 0) * 1e3 : getRetryDelay(attemptCount);
          await res$1.text();
          attemptCount++;
          await sleep(delay, _this.signal);
          continue;
...
// src/auth.js
function createAuth(supabase2) {
  return {
    getCustomSession: /* @__PURE__ */ __name((token) => getCustomSession(supabase2, token), "getCustomSession"),
    createNotionSession: /* @__PURE__ */ __name((opts) => createNotionSession(supabase2, opts), "createNotionSession"),
    deleteCustomSession: /* @__PURE__ */ __name((token) => deleteCustomSession(supabase2, token), "deleteCustomSession"),
    getActiveInstallation: /* @__PURE__ */ __name((userId) => getActiveInstallation(supabase2, userId), "getActiveInstallation"),
    getActiveInstallationWithTokens: /* @__PURE__ */ __name((userId) => getActiveInstallationWithTokens(supabase2, userId), "getActiveInstallationWithTokens"),
    revokeInstallation: /* @__PURE__ */ __name((userId) => revokeInstallation(supabase2, userId), "revokeInstallation")
  };
}
__name(createAuth, "createAuth");
async function getCustomSession(supabase2, token) {
  if (!token) return null;
  const { data, error: error3 } = await supabase2.from("session").select("id, token, expiresAt, userId, user:userId(id, name, email, image, emailVerified)").eq("token", token).gt("expiresAt", (/* @__PURE__ */ new Date()).toISOString()).single();
  if (error3 || !data) return null;
  return {
    session: {
      id: data.id,
      token: data.token,
...
__name(getCustomSession, "getCustomSession");
async function createNotionSession(supabase2, {
  userId,
  notionAccountId,
  accessToken,
  refreshToken,
  ...
  sessionToken,
  sessionId,
  SESSION_MAX_AGE_SECONDS: SESSION_MAX_AGE_SECONDS2
}) {
  userId = await upsertJotUser(supabase2, userId, user);
  const accountId = `notion:${notionAccountId}`;
  const expiresAt2 = new Date(Date.now() + SESSION_MAX_AGE_SECONDS2 * 1e3).toISOString();
  await supabase2.from("account").upsert(
    {
  ...
  await supabase2.from("session").delete().or(`userId.eq.${userId},expiresAt.lt.${(/* @__PURE__ */ new Date()).toISOString()}`);
  await supabase2.from("session").insert({
    id: sessionId,
  ...
  const installationId = await upsertNotionInstallation(supabase2, userId, {
    workspaceId: workspaceId ?? null,
    workspaceName: workspaceName ?? null
  });
  return { sessionToken, installationId };
}
__name(createNotionSession, "createNotionSession");
async function deleteCustomSession(supabase2, token) {
  if (!token) return;
  await supabase2.from("session").delete().eq("token", token);
}
__name(deleteCustomSession, "deleteCustomSession");
async function revokeInstallation(supabase2, userId) {
  await supabase2.from("notion_installations").update({ active: 0, revoked_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("user_id", userId);
}
__name(revokeInstallation, "revokeInstallation");
async function upsertJotUser(supabase2, preferredUserId, user) {
  const { data: existing } = await supabase2.from("user").select("id").or(`id.eq.${preferredUserId},email.eq.${user.email}`).limit(1).maybeSingle();
  const userId = existing?.id ?? preferredUserId;
  await supabase2.from("user").upsert(
    { id: userId, name: user.name, email: user.email, emailVerified: 0, image: user.image },
    { onConflict: "id" }
  );
  return userId;
}
__name(upsertJotUser, "upsertJotUser");
async function upsertNotionInstallation(supabase2, userId, { workspaceId, workspaceName }) {
  await revokeInstallation(supabase2, userId);
  const { data } = await supabase2.from("notion_installations").upsert(
    {
      user_id: userId,
...
__name(upsertNotionInstallation, "upsertNotionInstallation");
async function getActiveInstallation(supabase2, userId) {
  const { data } = await supabase2.from("notion_installations").select("id, workspace_id, workspace_name").eq("user_id", userId).eq("active", 1).order("updated_at", { ascending: false }).limit(1).maybeSingle();
  return data;
}
__name(getActiveInstallation, "getActiveInstallation");
async function getActiveInstallationWithTokens(supabase2, userId) {
  const [{ data: accountRow }, { data: installRow }] = await Promise.all([
    supabase2.from("account").select("accessToken").eq("userId", userId).eq("providerId", "notion").limit(1).maybeSingle(),
    supabase2.from("notion_installations").select("id, workspace_id, workspace_name").eq("user_id", userId).eq("active", 1).order("updated_at", { ascending: false }).limit(1).maybeSingle()
  ]);
  if (!accountRow?.accessToken || !installRow) return void 0;
  return { ...installRow, tokens: { access_token: accountRow.accessToken } };
}
__name(getActiveInstallationWithTokens, "getActiveInstallationWithTokens");

// src/blockConversion.js
function tiptapDocumentToNotionBlocks(doc) {
  return (doc?.content ?? []).map((node) => tiptapNodeToNotionBlock(node));
}
__name(tiptapDocumentToNotionBlocks, "tiptapDocumentToNotionBlocks");
function tiptapNodeToNotionBlock(node) {
  const richText2 = inlineContentToRichText(node.content);
    ...
    return {
      ...
      paragraph: {
        rich_text: plainRichText(
          (node.content ?? []).map((item) => `${item.attrs?.checked ? "[x]" : "[ ]"} ${textFromNode(item)}`).join("\n")
        ),
        color: "default"
      }
    };
  ...
  return paragraphFallback(richText2.length ? richText2 : plainRichText(""));
}
__name(tiptapNodeToNotionBlock, "tiptapNodeToNotionBlock");
function paragraphFallback(richText2) {
  const rt = typeof richText2 === "string" ? plainRichText(richText2) : richText2;
  return {
    object: "block",
    type: "paragraph",
    paragraph: {
...
__name(paragraphFallback, "paragraphFallback");
function inlineContentToRichText(content = []) {
  const richText2 = [];
  for (const node of content) {
    if (node.type === "text") {
      richText2.push(textNodeToRichText(node));
    } else if (node.type === "hardBreak") {
      richText2.push(...plainRichText("\n"));
    } else {
      richText2.push(...inlineContentToRichText(node.content ?? []));
    }
  }
  return richText2.length ? richText2 : [];
}
__name(inlineContentToRichText, "inlineContentToRichText");
...
__name(textNodeToRichText, "textNodeToRichText");
...
__name(plainRichText, "plainRichText");
function notionBlocksToTiptapDocument(blocks) {
  return {
    type: "doc",
    content: blocks.map(notionBlockToTiptapNode).filter(Boolean)
  };
}
__name(notionBlocksToTiptapDocument, "notionBlocksToTiptapDocument");
function notionBlocksToTiptapDocumentStrict(blocks) {
  const content = [];
...
__name(notionBlocksToTiptapDocumentStrict, "notionBlocksToTiptapDocumentStrict");
...
__name(notionBlockToTiptapNode, "notionBlockToTiptapNode");
function richTextToTiptapInline(richText2 = []) {
  const content = [];
  for (const item of richText2) {
...
__name(richTextToTiptapInline, "richTextToTiptapInline");
...
__name(marksFromRichText, "marksFromRichText");
...
__name(plainTiptapText, "plainTiptapText");
...
function normalizeYoutubeVideoUrl(src) {
  try {
    ...
    if (host.endsWith("youtu.be")) {
      ...
      return id2 ? youtubeWatchUrl(id2, url.searchParams) : src;
    }
...
__name(normalizeYoutubeVideoUrl, "normalizeYoutubeVideoUrl");
...
__name(youtubeWatchUrl, "youtubeWatchUrl");
function kindFromNotionBlock(block) {
...
__name(kindFromNotionBlock, "kindFromNotionBlock");

// src/importManagedBlocks.js
import { randomUUID, createHash } from "node:crypto";
var JOT_BLOCK_ID_ATTR = "jotBlockId";
async function importManagedBlocks({
  store,
  page,
  listAllBlockChildren: listAllBlockChildren2,
  hash: hash2 = defaultHash
}) {
  const children = await listAllBlockChildren2(store, page.notionPageId);
...
__name(importManagedBlocks, "importManagedBlocks");
function rebuildImportedBlockMappings({
  ...
  const existingByNotionId = new Map(
    (store.blockMappings[localPageId] ?? []).filter((mapping) => mapping.notionBlockId).map((mapping) => [mapping.notionBlockId, mapping])
  );
  const content = imported.content.map((node, index) => {
    const existing = existingByNotionId.get(notionBlocks[index]?.id);
    const jotBlockId = existing?.jotBlockId ?? existing?.localNodeId ?? `jot-block-${randomUUID()}`;
    return {
      ...node,
      attrs: {
        ...node.attrs ?? {},
  ...
  store.blockMappings[localPageId] = outboundBlocks.map((notionBlock, index) => {
    ...
    return {
      ...
      order: index,
      lastSyncedHash: hash2(JSON.stringify(notionBlock ?? {})),
...
__name(rebuildImportedBlockMappings, "rebuildImportedBlockMappings");
function defaultHash(value) {
  return createHash("sha256").update(value).digest("hex");
}
__name(defaultHash, "defaultHash");

// src/managedBlocks.js
var JOT_BLOCK_ID_ATTR2 = "jotBlockId";
var UPDATEABLE_NOTION_TYPES = /* @__PURE__ */ new Set([
  "paragraph",
  "quote",
  "code",
  "heading_1",
...
async function replaceManagedBlocks({
  ...
  content,
  listAllBlockChildren: listAllBlockChildren2,
  deleteManagedBlock: deleteManagedBlock2,
  appendManagedBlocks: appendManagedBlocks2,
  updateManagedBlock: updateManagedBlock2,
  tiptapDocumentToNotionBlocks: tiptapDocumentToNotionBlocks2,
  kindFromNotionBlock: kindFromNotionBlock2,
  hash: hash2
}) {
  const notionBlocks = tiptapDocumentToNotionBlocks2(content);
...
__name(replaceManagedBlocks, "replaceManagedBlocks");
async function replaceAllManagedBlocks({
  ...
  const existingBlocks = await listAllBlockChildren2(store, notionPageId);
  for (const block of existingBlocks) {
    await deleteManagedBlock2(store, block.id);
  }
  const createdBlocks = await appendManagedBlocks2(store, notionPageId, notionBlocks);
...
__name(replaceAllManagedBlocks, "replaceAllManagedBlocks");
async function reconcileManagedBlocks({
  ...
  for (const entry of desiredBlocks) {
    ...
    if (!existing?.notionBlockId) {
      const [createdBlock2] = await appendManagedBlocks2(
        store,
        notionPageId,
        [entry.notionBlock],
        positionAfter(previousNotionBlockId)
      );
      createdByOrder[entry.order] = createdBlock2;
      nextMappings.push(mappingFromDesired(entry, createdBlock2?.id));
      previousNotionBlockId = createdBlock2?.id ?? previousNotionBlockId;
      continue;
    }
    ...
    if (isUpdateCompatible(existing, entry)) {
      await updateManagedBlock2(store, existing.notionBlockId, entry.notionBlock);
      nextMappings.push(mappingFromDesired(entry, existing.notionBlockId, existing));
      previousNotionBlockId = existing.notionBlockId;
      continue;
    }
    await deleteManagedBlock2(store, existing.notionBlockId);
    const [createdBlock] = await appendManagedBlocks2(
      store,
      notionPageId,
      [entry.notionBlock],
      positionAfter(previousNotionBlockId)
  ...
  for (const mapping of existingMappings) {
    if (!desiredIds.has(mappingKey(mapping)) && mapping.notionBlockId) {
      await deleteManagedBlock2(store, mapping.notionBlockId);
    }
  }
...
__name(reconcileManagedBlocks, "reconcileManagedBlocks");
async function rebuildReorderedRange({
  ...
  const oldAffectedOrders = existingMappings.filter((mapping) => affectedIds.has(mappingKey(mapping))).map((mapping) => mapping.order ?? Number.MAX_SAFE_INTEGER);
  ...
  const previousMapping = desiredBlocks.slice(0, range.start).reverse().map((entry) => existingByJotId.get(entry.jotBlockId)).find((mapping) => mapping?.notionBlockId);
  const createdByOrder = [];
...
__name(rebuildReorderedRange, "rebuildReorderedRange");
function desiredManagedBlocks({
  ...
  return notionBlocks.map((notionBlock, index) => {
    ...
    return {
      ...
      localPageId,
      notionBlock,
      kind: kindFromNotionBlock2(notionBlock),
      lastSyncedHash: hash2(JSON.stringify(notionBlock ?? {})),
      order: index
    };
  });
}
__name(desiredManagedBlocks, "desiredManagedBlocks");
function jotBlockIdFromNode(node, index) {
  const value = node?.attrs?.[JOT_BLOCK_ID_ATTR2];
  return typeof value === "string" && value ? value : `block-${index}`;
}
__name(jotBlockIdFromNode, "jotBlockIdFromNode");
...
__name(mappingFromDesired, "mappingFromDesired");
...
__name(mappingsByJotId, "mappingsByJotId");
function mappingKey(mapping) {
  return mapping.jotBlockId ?? mapping.localNodeId;
}
__name(mappingKey, "mappingKey");
function hasReorderedManagedBlocks(existingMappings, desiredBlocks) {
  ...
  const newCommon = desiredBlocks.map((entry) => entry.jotBlockId).filter((id) => existingIds.has(id));
  return oldCommon.length > 1 && oldCommon.join("\n") !== newCommon.join("\n");
}
__name(hasReorderedManagedBlocks, "hasReorderedManagedBlocks");
function reorderedRange(existingMappings, desiredBlocks) {
  ...
  const indexes = desiredBlocks.map((entry, index) => affectedIds.has(entry.jotBlockId) ? index : -1).filter((index) => index >= 0);
  return {
    start: Math.min(...indexes),
    end: Math.max(...indexes)
  };
}
__name(reorderedRange, "reorderedRange");
...
__name(isUpdateCompatible, "isUpdateCompatible");
...
__name(positionAfter, "positionAfter");

// src/notionRequest.js
var DEFAULT_REQUESTS_PER_SECOND = 3;
var DEFAULT_MAX_RETRIES2 = 3;
var DEFAULT_BASE_BACKOFF_MS = 250;
var DEFAULT_MAX_BACKOFF_MS = 2e3;
function createNotionRequester({
  ...
  return /* @__PURE__ */ __name(async function notionRequest2(store, endpoint, init = {}) {
    let attempt = 0;
    while (true) {
      ...
      try {
        ...
        const error3 = createNotionError(response, payload);
        if (!shouldRetryError(error3) || attempt >= maxRetries) {
          throw error3;
        }
        await sleep3(retryDelayMs(error3, attempt, { baseBackoffMs, maxBackoffMs }));
      } catch (error3) {
        if (isNotionError(error3) || attempt >= maxRetries) {
          throw error3;
        }
        await sleep3(backoffDelayMs(attempt, { baseBackoffMs, maxBackoffMs }));
      }
      attempt += 1;
    }
  }, "notionRequest");
}
__name(createNotionRequester, "createNotionRequester");
async function uploadFileToNotion(store, { data, mimeType, filename }, { notionVersion, baseUrl = "https://api.notion.com/v1", fetchImpl = globalThis.fetch } = {}) {
...
__name(uploadFileToNotion, "uploadFileToNotion");
...
function createNotionError(response, payload) {
  const error3 = new Error(payload.message ?? `Notion returned ${response.status}.`);
  error3.status = response.status;
  error3.code = payload.code;
  error3.retryAfter = retryAfterHeader(response);
  error3.isNotionApiError = true;
  return error3;
}
__name(createNotionError, "createNotionError");
function retryAfterHeader(response) {
  const value = response.headers?.get?.("Retry-After");
  const seconds = Number(value);
  return Number.isFinite(seconds) && seconds >= 0 ? seconds : void 0;
}
__name(retryAfterHeader, "retryAfterHeader");
function shouldRetryError(error3) {
  if (error3.status === 429 && error3.code === "rate_limited") {
    return true;
  }
  return [500, 502, 503, 504].includes(error3.status);
}
__name(shouldRetryError, "shouldRetryError");
function retryDelayMs(error3, attempt, options) {
  if (error3.status === 429 && error3.retryAfter !== void 0) {
    return error3.retryAfter * 1e3;
  }
...
function isNotionError(error3) {
  return Boolean(error3?.isNotionApiError);
}
__name(isNotionError, "isNotionError");
...
// src/pageSync.js
async function pushPageToNotionCore({
  request,
  page,
  project,
  ...
  const {
    archiveThreadToggle: archiveThreadToggle2,
    appendLog: appendLog2,
    createChildPage: createChildPage2,
    ensureJotRootPage: ensureJotRootPage2,
    ensureProjectPage: ensureProjectPage2,
    ensureProjectRootPage: ensureProjectRootPage2,
    ensureThreadToggle: ensureThreadToggle2,
    notionRequest: notionRequest2,
    replaceManagedBlocks: replaceManagedBlocks3,
    requireConnectedStore: requireConnectedStore2,
    updateThreadToggleTitle: updateThreadToggleTitle2,
    updateChildNotePage: updateChildNotePage2,
    writeStore: writeStore2
  } = dependencies;
  const store = await requireConnectedStore2(request);
  if (ensureProjectPage2 && ensureThreadToggle2) {
  ...
  const jotRootPage = await ensureJotRootPage2(store, { selectedParentPageId });
  const projectRootPage = await ensureProjectRootPage2(store, jotRootPage.id, project, {
    candidateNotionPageId: page.notionParentPageId
  });
  const notionPageId = page.notionPageId ?? store.notePages[page.id]?.notionPageId;
  let notePage = notionPageId ? await notionRequest2(store, `/pages/${notionPageId}`) : void 0;
  if (notePage?.last_edited_time && page.remoteRevision && notePage.last_edited_time !== page.remoteRevision) {
    appendLog2(store, "sync_stale", `${page.title} changed in Notion.`);
    await writeStore2(store);
    return {
      page: {
        ...page,
        notionPageId,
        notionDatabaseId: void 0,
        notionDataSourceId: void 0,
  ...
  if (!notePage) {
    notePage = await createChildPage2(store, projectRootPage.id, page.title);
  } else {
    await updateChildNotePage2(store, notePage.id, page);
  }
  const replacement = await replaceManagedBlocks3(store, page.id, notePage.id, page.content);
  const refreshedPage = await notionRequest2(store, `/pages/${notePage.id}`);
  ...
  appendLog2(store, "sync_push", page.title);
  await writeStore2(store);
  return {
...
__name(pushPageToNotionCore, "pushPageToNotionCore");
async function pushPageToProjectDatabase({
  ...
  const projectPage = await ensureProjectPage2(store, project, { selectedParentPageId });
  if (page.status === "archived") {
    await archiveThreadToggle2?.(store, page);
    store.notePages[page.id] = {
      ...store.notePages[page.id] ?? {},
    ...
    appendLog2(store, "sync_thread_archived", page.title);
    await writeStore2(store);
    return {
      projectPage,
      page: {
  ...
  const toggle = await ensureThreadToggle2(store, projectPage.id, page);
  if (toggle?.last_edited_time && page.remoteRevision && toggle.last_edited_time !== page.remoteRevision) {
    appendLog2(store, "sync_stale", `${page.title} changed in Notion.`);
    await writeStore2(store);
    return {
      projectPage,
  ...
  await updateThreadToggleTitle2?.(store, page);
  const replacement = await replaceManagedBlocks3(store, page.id, toggle.id, page.content);
  const refreshedBlock = await notionRequest2(store, `/blocks/${toggle.id}`).catch(() => toggle);
  const syncedContent = normalizeSyncedMediaContent(
    page.content,
    replacement?.createdBlocks ?? []
  );
  ...
  appendLog2(store, "sync_thread_push", page.title);
  await writeStore2(store);
  return {
    projectPage,
    page: syncedPage,
...
__name(pushPageToProjectDatabase, "pushPageToProjectDatabase");
function normalizeSyncedMediaContent(content, createdBlocks) {
  ...
  return {
    ...
    content: content.content.map(
      (node, index) => normalizeSyncedMediaNode(node, createdBlocks[index])
    )
  };
}
__name(normalizeSyncedMediaContent, "normalizeSyncedMediaContent");
...
__name(normalizeSyncedMediaNode, "normalizeSyncedMediaNode");
...
__name(mediaUrlFromNotionBlock, "mediaUrlFromNotionBlock");

// src/projectDatabase.js
var JOT_DATABASE_TITLE = "Jot";
var PROJECT_STATE_BLOCK_KEY = "project-state";
...
function createProjectDatabaseHelpers({
  appendLog: appendLog2,
  createWorkspacePage: createWorkspacePage2,
  hash: hash2,
  isNotionObjectNotFound: isNotionObjectNotFound2,
  listAllBlockChildren: listAllBlockChildren2,
  notionBlocksToTiptapDocument: notionBlocksToTiptapDocument2,
  notionRequest: notionRequest2,
  replaceManagedBlocks: replaceManagedBlocks3,
  tiptapDocumentToNotionBlocks: tiptapDocumentToNotionBlocks2
}) {
  async function ensureProjectDatabase2(store, { selectedParentPageId } = {}) {
    store.projectPages ??= {};
    store.projectBlocks ??= {};
    store.threadBlocks ??= {};
    store.blockMappings ??= {};
    const ignoredDatabaseIds = store.ignoredJotDatabaseIds ?? /* @__PURE__ */ new Set();
    ...
    if (!selectedParentPageId) {
      ...
      if (foundAccessible) {
        ...
        appendLog2(store, "project_database_adopted", foundAccessible.title);
        return foundAccessible;
      }
    }
    const parentPage = selectedParentPageId ? await ensureDatabaseParentPage(store, selectedParentPageId) : void 0;
    const found = parentPage ? await findExistingDatabase(store, { parentPageId: parentPage.id, ignoredDatabaseIds }) : void 0;
    if (found) {
      store.jotDatabase = found;
      await ensureProjectSchema(store, found.dataSourceId);
      await ensureProjectViews(store, found);
      appendLog2(store, "project_database_adopted", found.title);
      return found;
    }
    ...
    appendLog2(store, "project_database_created", created.title);
    return created;
  }
  __name(ensureProjectDatabase2, "ensureProjectDatabase");
  async function ensureDatabaseParentPage(store, selectedParentPageId) {
    if (selectedParentPageId) {
      try {
        const page2 = await notionRequest2(store, `/pages/${selectedParentPageId}`);
        store.jotRootPage = {
          id: page2.id,
          parentPageId: void 0,
          title: titleFromPage(page2) || JOT_DATABASE_TITLE,
          url: page2.url
        };
        return store.jotRootPage;
      } catch (error3) {
        if (!isNotionObjectNotFound2(error3) && !isBlockNotPageError(error3)) {
          throw error3;
        }
        appendLog2(store, "database_parent_inaccessible", `${selectedParentPageId}: ${error3.message}`);
      }
      return void 0;
    }
    if (store.jotRootPage?.id) {
      try {
        const page2 = await notionRequest2(store, `/pages/${store.jotRootPage.id}`);
        store.jotRootPage = {
          id: page2.id,
          parentPageId: void 0,
          title: titleFromPage(page2) || store.jotRootPage.title || JOT_DATABASE_TITLE,
          url: page2.url
        };
        return store.jotRootPage;
      } catch {
    ...
    const page = await createWorkspacePage2(store, JOT_DATABASE_TITLE);
    store.jotRootPage = {
      id: page.id,
      parentPageId: void 0,
    ...
    appendLog2(store, "database_parent_created", store.jotRootPage.title);
    return store.jotRootPage;
  }
  __name(ensureDatabaseParentPage, "ensureDatabaseParentPage");
  async function refreshDatabase(store, stored) {
    try {
      const database = await notionRequest2(store, `/databases/${stored.databaseId}`);
      ...
      appendLog2(store, "project_database_lookup_error", error3.message);
      return void 0;
    }
  }
  __name(refreshDatabase, "refreshDatabase");
  async function findExistingDatabase(store, { parentPageId, ignoredDatabaseIds = /* @__PURE__ */ new Set() } = {}) {
    const response = await notionRequest2(store, "/search", {
      method: "POST",
      body: {
    ...
    for (const database of response.results ?? []) {
      ...
      if (!ignoredDatabaseIds.has(database.id) && !isArchivedObject(database) && titleFromDatabase(database) === JOT_DATABASE_TITLE && (parentPageId === void 0 || parentPageIdFromObject(database) === parentPageId) && dataSourceId) {
        return databaseSummary(database, dataSourceId, parentPageId ?? parentPageIdFromObject(database));
      }
    }
    return void 0;
  }
  __name(findExistingDatabase, "findExistingDatabase");
  async function createProjectDatabaseWithFallback(store, parentPageId, {
    ignoredDatabaseIds = /* @__PURE__ */ new Set()
  } = {}) {
    if (parentPageId) {
    ...
    try {
      ...
      appendLog2(store, "project_database_workspace_create_error", error3.message);
  ...
  __name(createProjectDatabaseWithFallback, "createProjectDatabaseWithFallback");
  async function createProjectDatabase(store, parentPageId) {
    const database = await notionRequest2(store, "/databases", {
      method: "POST",
      body: {
    ...
    const fullDatabase = await notionRequest2(store, `/databases/${database.id}`).catch(() => database);
  ...
  __name(createProjectDatabase, "createProjectDatabase");
  async function ensureProjectSchema(store, dataSourceId) {
    const dataSource = await notionRequest2(store, `/data_sources/${dataSourceId}`);
    ...
    if (Object.keys(missing).length) {
      await notionRequest2(store, `/data_sources/${dataSourceId}`, {
        method: "PATCH",
        body: { properties: missing }
      });
    }
  }
  __name(ensureProjectSchema, "ensureProjectSchema");
  async function ensureProjectViews(store, database) {
    ...
    const dataSource = await notionRequest2(store, `/data_sources/${database.dataSourceId}`).catch(() => ({}));
    ...
    for (const view of views) {
      ...
      const created = await notionRequest2(store, "/views", {
        method: "POST",
        body: {
        ...
        appendLog2(store, "project_view_create_error", `${view.name}: ${error3.message}`);
        return void 0;
      });
      if (created?.id) {
  ...
  __name(ensureProjectViews, "ensureProjectViews");
  async function listViews(store, databaseId) {
    ...
    do {
      ...
      const response = await notionRequest2(store, `/views?${search}`).catch(() => ({ results: [] }));
      for (const viewRef of response.results ?? []) {
        if (viewRef.name) {
          results.push(viewRef);
        } else if (viewRef.id) {
          const view = await notionRequest2(store, `/views/${viewRef.id}`).catch(() => viewRef);
          results.push(view);
        }
      }
  ...
  __name(listViews, "listViews");
  async function ensureProjectPage2(store, project, { selectedParentPageId } = {}) {
    return ensureProjectPageAttempt(store, project, { selectedParentPageId });
  }
  __name(ensureProjectPage2, "ensureProjectPage");
  async function ensureProjectPageAttempt(store, project, { selectedParentPageId, retryOnArchivedAncestor = true } = {}) {
    const database = await ensureProjectDatabase2(store, { selectedParentPageId });
    const stored = store.projectPages?.[project.id];
    const existingPageId = stored?.notionPageId;
    let page = existingPageId ? await notionRequest2(store, `/pages/${existingPageId}`).catch(() => void 0) : void 0;
    if (!page) {
    ...
    try {
      if (!page) {
        ...
        appendLog2(store, "project_database_page_created", project.name || "Untitled Project");
      } else {
      ...
      if (!retryOnArchivedAncestor || !isArchivedAncestorError(error3)) {
        throw error3;
      }
      invalidateProjectDatabase(store, database.databaseId);
      appendLog2(store, "project_database_archived_ancestor", database.title);
  ...
  __name(ensureProjectPageAttempt, "ensureProjectPageAttempt");
  async function reloadProjectDatabaseFromNotion2(store, { selectedParentPageId } = {}) {
    clearSyncMappings(store);
    const requestedParentPageId = selectedParentPageId;
    const database = await ensureProjectDatabase2(store, { selectedParentPageId });
    const clearSelectedParentPage = Boolean(
      requestedParentPageId && store.jotRootPage?.id && store.jotRootPage.id !== requestedParentPageId
    );
    ...
    for (const row of rows) {
      ...
      const children = await listAllBlockChildren2(store, row.id).catch(() => []);
      const stateBlock = children.find(
        (block) => block.type === "toggle" && !block.archived && toggleTitle(block) === "Project State"
      );
      const threadBlocks = children.filter(
        (block) => block.type === "toggle" && !block.archived && toggleTitle(block) !== "Project State"
      );
      const importedProject = stateBlock ? await importProjectState(store, row, project, stateBlock) : project;
  ...
  __name(reloadProjectDatabaseFromNotion2, "reloadProjectDatabaseFromNotion");
  ...
  __name(clearSyncMappings, "clearSyncMappings");
  function invalidateProjectDatabase(store, databaseId) {
    store.ignoredJotDatabaseIds ??= /* @__PURE__ */ new Set();
    if (databaseId) {
      store.ignoredJotDatabaseIds.add(databaseId);
    }
    store.jotDatabase = void 0;
    clearSyncMappings(store);
  }
  __name(invalidateProjectDatabase, "invalidateProjectDatabase");
  async function queryManagedProjectRows(store, dataSourceId) {
    ...
    do {
      ...
      const response = await notionRequest2(store, `/data_sources/${dataSourceId}/query`, {
        method: "POST",
        body
      });
  ...
  __name(queryManagedProjectRows, "queryManagedProjectRows");
  function projectFromNotionPage(page) {
    ...
    const createdAt = dateProperty(properties[PROJECT_PROPERTIES.created]) ?? page.created_time ?? page.last_edited_time ?? (/* @__PURE__ */ new Date()).toISOString();
    const updatedAt = dateProperty(properties[PROJECT_PROPERTIES.updated]) ?? page.last_edited_time ?? createdAt;
    const category = selectProperty(properties[PROJECT_PROPERTIES.category]);
    const status = selectProperty(properties[PROJECT_PROPERTIES.status]) === "Archived" ? "archived" : "active";
    return {
      id,
      ...
      stateRemoteRevision: void 0,
      tags: category ? [category] : [],
      syncState: "saved"
    };
  }
  __name(projectFromNotionPage, "projectFromNotionPage");
  async function importProjectState(store, projectPage, project, stateBlock) {
    const blocks = await listAllBlockChildren2(store, stateBlock.id).catch(() => []);
    const content = blocks.length ? notionBlocksToTiptapDocument2(blocks) : emptyDocument();
  ...
  __name(importProjectState, "importProjectState");
  async function pageFromThreadBlock(store, project, projectPage, threadBlock) {
    const id = `page-${project.id}-${threadBlock.id}`;
    const title2 = toggleTitle(threadBlock) || "Untitled Page";
    const contentBlocks = await listAllBlockChildren2(store, threadBlock.id).catch(() => []);
    const content = contentBlocks.length ? notionBlocksToTiptapDocument2(contentBlocks) : emptyDocument();
  ...
  __name(pageFromThreadBlock, "pageFromThreadBlock");
  ...
  __name(findProjectPageByJotId, "findProjectPageByJotId");
  async function createProjectDatabasePage(store, dataSourceId, project) {
    return notionRequest2(store, "/pages", {
      method: "POST",
      body: {
        parent: {
          type: "data_source_id",
  ...
  __name(createProjectDatabasePage, "createProjectDatabasePage");
  async function updateProjectDatabasePage(store, pageId, project) {
    ...
    return notionRequest2(store, `/pages/${pageId}`, {
      method: "PATCH",
      body
    });
  }
  __name(updateProjectDatabasePage, "updateProjectDatabasePage");
  async function syncProjectState(store, notionPageId, project) {
    ...
    const hasRemoteEdit = previous?.lastEditedTime && container.last_edited_time && container.last_edited_time !== previous.lastEditedTime;
    if (hasRemoteEdit) {
      const blocks = await listAllBlockChildren2(store, container.id);
      const content = blocks.length ? notionBlocksToTiptapDocument2(blocks) : emptyDocument();
    ...
    await replaceManagedBlocks3(store, key, container.id, stateContent);
    const refreshed = await notionRequest2(store, `/blocks/${container.id}`).catch(() => container);
    store.projectBlocks[key] = {
      ...store.projectBlocks[key],
      blockId: container.id,
  ...
  __name(syncProjectState, "syncProjectState");
  async function ensureThreadToggle2(store, projectPageId, page) {
    return ensureToggleBlock(store, projectPageId, page.title || "Untitled Page", {
      key: threadKey(page.id),
      mappings: store.threadBlocks
    });
  }
  __name(ensureThreadToggle2, "ensureThreadToggle");
  async function ensureToggleBlock(store, parentBlockId, title2, { key, mappings }) {
    const stored = mappings?.[key];
    if (stored?.blockId) {
      const block = await notionRequest2(store, `/blocks/${stored.blockId}`).catch(() => void 0);
      if (block?.id && !block.archived) {
        if (toggleTitle(block) !== title2) {
          await updateToggleTitle(store, block.id, title2);
        }
        return { ...block, id: block.id };
      }
    }
    const children = await listAllBlockChildren2(store, parentBlockId).catch(() => []);
    const matching = children.find(
      (block) => block.type === "toggle" && !block.archived && toggleTitle(block) === title2
    );
    if (matching) {
      mappings[key] = { blockId: matching.id, parentPageId: parentBlockId, title: title2 };
      return matching;
    }
    const response = await notionRequest2(store, `/blocks/${parentBlockId}/children`, {
      method: "PATCH",
      body: {
        children: [toggleBlock(title2)]
      }
    });
    const created = response.results?.[0];
    if (!created?.id) {
      throw new Error(`Unable to create Notion toggle for ${title2}.`);
    }
    mappings[key] = { blockId: created.id, parentPageId: parentBlockId, title: title2 };
    return created;
  }
  __name(ensureToggleBlock, "ensureToggleBlock");
  async function updateToggleTitle(store, blockId, title2) {
    return notionRequest2(store, `/blocks/${blockId}`, {
      method: "PATCH",
      body: {
        toggle: {
          rich_text: richText(title2 || "Untitled Page")
        }
      }
    });
  }
  __name(updateToggleTitle, "updateToggleTitle");
  async function updateThreadToggleTitle2(store, page) {
  ...
  __name(updateThreadToggleTitle2, "updateThreadToggleTitle");
  async function archiveThreadToggle2(store, page) {
    const stored = store.threadBlocks?.[threadKey(page.id)];
    if (!stored?.blockId) {
      return;
    }
    await notionRequest2(store, `/blocks/${stored.blockId}`, {
      method: "DELETE"
    });
    delete store.threadBlocks[threadKey(page.id)];
    delete store.blockMappings[page.id];
  }
  __name(archiveThreadToggle2, "archiveThreadToggle");
  async function importThreadContent(store, page) {
    const stored = store.threadBlocks?.[threadKey(page.id)];
    if (!stored?.blockId) {
      return null;
    }
    const blocks = await listAllBlockChildren2(store, stored.blockId);
    return blocks.length ? notionBlocksToTiptapDocument2(blocks) : emptyDocument();
  }
  __name(importThreadContent, "importThreadContent");
  ...
  __name(storeProjectPage, "storeProjectPage");
  return {
    archiveThreadToggle: archiveThreadToggle2,
    ensureProjectDatabase: ensureProjectDatabase2,
    ensureProjectPage: ensureProjectPage2,
    ensureThreadToggle: ensureThreadToggle2,
    importThreadContent,
    reloadProjectDatabaseFromNotion: reloadProjectDatabaseFromNotion2,
    projectStateKey,
    threadKey,
    updateThreadToggleTitle: updateThreadToggleTitle2
  };
}
__name(createProjectDatabaseHelpers, "createProjectDatabaseHelpers");
function projectProperties(project) {
  const createdAt = project.createdAt ?? (/* @__PURE__ */ new Date()).toISOString();
...
__name(projectProperties, "projectProperties");
...
__name(activeProjectsView, "activeProjectsView");
...
__name(byCategoryView, "byCategoryView");
...
__name(allProjectsView, "allProjectsView");
...
__name(tableConfiguration, "tableConfiguration");
...
__name(databaseSummary, "databaseSummary");
...
__name(pageSummaryFromNotionPage, "pageSummaryFromNotionPage");
...
__name(databaseParent, "databaseParent");
function firstDataSourceId(database) {
  return database?.data_sources?.[0]?.id ?? database?.initial_data_source?.id;
}
__name(firstDataSourceId, "firstDataSourceId");
function parentPageIdFromObject(object) {
  return object?.parent?.type === "page_id" ? object.parent.page_id : void 0;
}
__name(parentPageIdFromObject, "parentPageIdFromObject");
function isArchivedObject(object) {
  return Boolean(object?.archived || object?.in_trash);
}
__name(isArchivedObject, "isArchivedObject");
function isArchivedAncestorError(error3) {
  return error3?.code === "validation_error" && /archived ancestor|unarchive the ancestor/i.test(error3?.message ?? "");
}
__name(isArchivedAncestorError, "isArchivedAncestorError");
function isBlockNotPageError(error3) {
  return error3?.code === "validation_error" && /is a block, not a page|retrieve block API/i.test(error3?.message ?? "");
}
__name(isBlockNotPageError, "isBlockNotPageError");
function titleFromDatabase(database) {
  return (database?.title ?? []).map((item) => item.plain_text ?? item.text?.content ?? "").join("");
}
__name(titleFromDatabase, "titleFromDatabase");
...
__name(projectStateKey, "projectStateKey");
function threadKey(pageId) {
...
async function syncProjectFolder({
  ...
  ensureProjectPage: ensureProjectPage2,
  ensureProjectRootPage: ensureProjectRootPage2,
  archiveProjectRootPage: archiveProjectRootPage2,
  pageSummary: pageSummary3
}) {
  if (project.status === "archived") {
    if (ensureProjectPage2) {
      const projectPage2 = await ensureProjectPage2(store, project, { selectedParentPageId });
      return {
        project: projectPage2.project,
    ...
    await archiveProjectRootPage2(store, stored.notionPageId);
    delete store.projectPages[project.id];
    return {
      projectPage: pageSummary3({
  ...
  const jotRootPage = await ensureJotRootPage2(store, { selectedParentPageId });
  const projectPage = await ensureProjectRootPage2(store, jotRootPage.id, project);
  return {
    parentPage: jotRootPage,
    projectPage,
    status: "saved",
...
__name(syncProjectFolder, "syncProjectFolder");

// src/rootPages.js
function createRootPageHelpers({
  appendLog: appendLog2,
  createChildPage: createChildPage2,
  createWorkspacePage: createWorkspacePage2,
  isNotionObjectNotFound: isNotionObjectNotFound2,
  ...
  listAllBlockChildren: listAllBlockChildren2,
  notionRequest: notionRequest2,
  titleFromPage: titleFromPage3,
  updatePageTitle: updatePageTitle2
}) {
  async function ensureJotRootPage2(store, { selectedParentPageId } = {}) {
    const stored = store.jotRootPage;
    if (selectedParentPageId) {
      if (stored?.id === selectedParentPageId) {
        const existing = await refreshStoredRootPage(store, stored, void 0);
        if (existing) {
          return existing;
        }
      }
      try {
        ...
        store.jotRootPage = {
          id: page2.id,
          parentPageId: void 0,
          title: titleFromPage3(page2) || jotRootPageTitle,
          url: page2.url
        };
        store.jotDatabase = void 0;
        appendLog2(store, "root_page_adopted", store.jotRootPage.title);
        return pageSummary(store.jotRootPage);
      } catch (error3) {
        if (!isNotionObjectNotFound2(error3) && !isBlockNotPageError2(error3)) {
          throw error3;
        }
        appendLog2(
          store,
          "root_parent_inaccessible",
          `${selectedParentPageId}: ${error3.message}`
        );
      }
    }
    if (stored?.id && !selectedParentPageId) {
    ...
    const page = await createWorkspacePage2(store, jotRootPageTitle);
    store.jotRootPage = {
      id: page.id,
      parentPageId: void 0,
      title: titleFromPage3(page) || jotRootPageTitle,
      url: page.url
    };
    store.jotDatabase = void 0;
    appendLog2(store, "root_page_created", store.jotRootPage.title);
    return pageSummary(store.jotRootPage);
  }
  __name(ensureJotRootPage2, "ensureJotRootPage");
  async function refreshStoredRootPage(store, stored, parentPageId) {
    try {
      const page = await notionRequest2(store, `/pages/${stored.id}`);
      store.jotRootPage = {
        id: page.id,
        parentPageId,
        title: titleFromPage3(page) || stored.title || jotRootPageTitle,
        url: page.url
      };
      return pageSummary(store.jotRootPage);
    } catch (error3) {
      appendLog2(store, "root_page_lookup_error", error3.message);
      return void 0;
    }
  }
  __name(refreshStoredRootPage, "refreshStoredRootPage");
  async function ensureProjectRootPage2(store, jotRootPageId, project, { candidateNotionPageId } = {}) {
    const stored = store.projectPages[project.id];
    ...
    const page = await createChildPage2(store, jotRootPageId, project.name || "Untitled Project");
    return storeProjectRootPage(store, jotRootPageId, project, page, "project_page_created");
  }
  __name(ensureProjectRootPage2, "ensureProjectRootPage");
  async function refreshProjectRootPage(store, jotRootPageId, project, stored) {
    try {
      const page = await notionRequest2(store, `/pages/${stored.notionPageId}`);
      const title2 = project.name || stored.title || "Untitled Project";
      if (titleFromPage3(page) !== title2) {
        await updatePageTitle2(store, page.id, title2);
      }
      ...
      appendLog2(store, "project_page_lookup_error", error3.message);
      return void 0;
    }
  }
  __name(refreshProjectRootPage, "refreshProjectRootPage");
  async function adoptCandidateProjectPage(store, jotRootPageId, project, candidateNotionPageId) {
    if (!candidateNotionPageId) {
      return void 0;
    }
    try {
      const page = await notionRequest2(store, `/pages/${candidateNotionPageId}`);
      if (parentPageIdFromNotionPage(page) !== jotRootPageId) {
        return void 0;
      }
      const title2 = project.name || titleFromPage3(page) || "Untitled Project";
      if (titleFromPage3(page) !== title2) {
        await updatePageTitle2(store, page.id, title2);
      }
      return storeProjectRootPage(store, jotRootPageId, project, {
      ...
      appendLog2(store, "project_page_candidate_error", error3.message);
      return void 0;
    }
  }
  __name(adoptCandidateProjectPage, "adoptCandidateProjectPage");
  async function findChildPageByTitle(store, jotRootPageId, title2) {
    const children = await listAllBlockChildren2(store, jotRootPageId).catch((error3) => {
      appendLog2(store, "project_page_lookup_error", error3.message);
      return [];
    });
    const child = children.find(
      (block) => block.type === "child_page" && block.child_page?.title === title2 && !block.archived
    );
    return child ? {
      id: child.id,
  ...
  __name(findChildPageByTitle, "findChildPageByTitle");
  function storeProjectRootPage(store, jotRootPageId, project, page, logEvent) {
    const title2 = page.title || titleFromPage3(page) || project.name || "Untitled Project";
    store.projectPages[project.id] = {
      notionPageId: page.id,
    ...
    if (logEvent) {
      appendLog2(store, logEvent, store.projectPages[project.id].title);
    }
    return pageSummary({
      id: page.id,
      parentPageId: jotRootPageId,
  ...
  __name(storeProjectRootPage, "storeProjectRootPage");
  return {
    ensureJotRootPage: ensureJotRootPage2,
    ensureProjectRootPage: ensureProjectRootPage2,
    pageSummary
...
__name(createRootPageHelpers, "createRootPageHelpers");
...
__name(pageSummary, "pageSummary");
function parentPageIdFromNotionPage(page) {
  return page?.parent?.type === "page_id" ? page.parent.page_id : void 0;
}
__name(parentPageIdFromNotionPage, "parentPageIdFromNotionPage");
function isBlockNotPageError2(error3) {
  return error3?.code === "validation_error" && /is a block, not a page|retrieve block API/i.test(error3?.message ?? "");
}
...
var MAX_MEDIA_UPLOAD_BYTES = 20 * 1024 * 1024;
var installationMutationLocks = /* @__PURE__ */ new Map();
var app = new Hono2();
app.use("*", async (c, next) => {
...
app.get(
  "/auth/jot/complete",
  (c) => c.html(closePage("You are logged in to Jot. You can return to the side panel."))
);
app.get(
  "/auth/jot/error",
  (c) => c.html(closePage("Jot login did not complete. You can close this tab and try again."), 400)
);
app.get("/auth/notion/start", async (c) => {
  const env2 = c.env;
  if (!env2.NOTION_OAUTH_CLIENT_ID || !env2.NOTION_OAUTH_CLIENT_SECRET) {
    return c.html(closePage("Notion login is not configured on the sync server yet."), 503);
  }
  const workerUrl = serverBaseUrl(env2);
  const state = randomToken(24);
  const redirectUri = `${workerUrl}/auth/notion/callback`;
  const url = new URL("https://api.notion.com/v1/oauth/authorize");
  url.searchParams.set("client_id", env2.NOTION_OAUTH_CLIENT_ID);
...
app.get("/auth/notion/callback", async (c) => {
  ...
  try {
    const tokens = await exchangeNotionCode(env2, code, `${serverBaseUrl(env2)}/auth/notion/callback`);
    ...
    setCookie(c, JOT_SESSION_COOKIE, sessionToken, {
      ...
      path: "/",
      sameSite: "Lax",
      secure: serverBaseUrl(env2).startsWith("https://")
    });
...
app.get("/session", async (c) => {
  ...
  const installation = session ? await auth.getActiveInstallation(session.user.id).catch(() => void 0) : void 0;
  return c.json({
...
app.post("/notion/pages", async (c) => {
  ...
  const title2 = String(body?.title ?? "").trim() || "Jot";
  const store = await requireConnectedStore(c);
  const page = await createWorkspacePage(store, title2);
  appendLog(store, "parent_page_created", title2);
  await writeStore(store);
  return c.json({ page: { id: page.id, title: titleFromPage2(page), url: page.url } });
});
...
app.post("/sync/pull", async (c) => {
  ...
  const pulledContent = await importManagedBlocks2(store, page);
  if (!pulledContent) {
    return c.json({
      page: {
        ...page,
...
var pageSummary2;
var archiveThreadToggle;
var ensureProjectDatabase;
var ensureProjectPage;
var ensureThreadToggle;
var reloadProjectDatabaseFromNotion;
var updateThreadToggleTitle;
function initSingletons(env2) {
  supabase = createClient(env2.SUPABASE_URL, env2.SUPABASE_SERVICE_KEY);
  ...
  syncQueue = new MergeableSyncQueue();
  ...
  const dbHelpers = createProjectDatabaseHelpers({
    ...
    notionRequest,
    replaceManagedBlocks: replaceManagedBlocks2,
  ...
  return withFreshInstallationStore(
    ...
    (freshStore) => pushPageToNotionCore({
      ...
      dependencies: {
        ...
        notionRequest,
        replaceManagedBlocks: replaceManagedBlocks2,
        requireConnectedStore: /* @__PURE__ */ __name(async () => freshStore, "requireConnectedStore"),
...
function appendLog(store, event, message) {
  store.logs.push({ at: (/* @__PURE__ */ new Date()).toISOString(), event, message });
...
__name(requireConnectedStore, "requireConnectedStore");
...
__name(withFreshInstallationStore, "withFreshInstallationStore");
...
__name(requireJotSession, "requireJotSession");
...
__name(ensureJotSyncStateRow, "ensureJotSyncStateRow");
async function readJotSyncState(installationId) {
  await ensureJotSyncStateRow(installationId);
  const { data: row } = await supabase.from("jot_sync_state").select("*").eq("installation_id", installationId).single();
  if (!row) return normalizeStore({});
  return {
    jotRootPage: row.jot_database_id && !row.jot_data_source_id ? { id: row.jot_database_id, parentPageId: row.jot_parent_page_id, title: row.jot_database_title } : void 0,
...
__name(readJotSyncState, "readJotSyncState");
...
__name(writeJotSyncState, "writeJotSyncState");
async function exchangeNotionCode(env2, code, redirectUri) {
  const credentials = Buffer.from(
    `${env2.NOTION_OAUTH_CLIENT_ID}:${env2.NOTION_OAUTH_CLIENT_SECRET}`
  ).toString("base64");
...
__name(exchangeNotionCode, "exchangeNotionCode");
...
async function validateNotionCache(store, { pages, projects }) {
  ...
  if (store.jotDatabase?.databaseId && !await notionObjectExists(store, "database", store.jotDatabase.databaseId)) {
    store.jotDatabase = void 0;
    store.projectPages = {};
    store.projectBlocks = {};
    store.threadBlocks = {};
  ...
  for (const page of pages) {
    ...
    if (notionPageId && !cachedThread?.blockId && !await notionObjectExists(store, cachedNote?.kind === "thread" ? "block" : "page", notionPageId)) {
      uncachePage(store, page.id);
      uncachedPageIds.add(page.id);
      changed = true;
    }
...
__name(validateNotionCache, "validateNotionCache");
...
__name(uncachePage, "uncachePage");
async function notionObjectExists(store, kind, id) {
  try {
    ...
    if (isNotionObjectNotFound(error3)) return false;
    throw error3;
  }
}
__name(notionObjectExists, "notionObjectExists");
async function retrieveNotionPageOrBlock(store, page) {
  ...
  try {
    ...
    if (!isBlockNotPageError3(error3)) throw error3;
    appendLog(store, "sync_pull_block_fallback", page.title ?? page.id);
    return notionRequest(store, `/blocks/${page.notionPageId}`);
  }
}
__name(retrieveNotionPageOrBlock, "retrieveNotionPageOrBlock");
function isThreadBackedPage(store, page) {
  const cachedThread = store.threadBlocks?.[threadKey(page.id)];
  const cachedNote = store.notePages?.[page.id];
  return Boolean(cachedThread?.blockId === page.notionPageId || cachedNote?.kind === "thread");
}
__name(isThreadBackedPage, "isThreadBackedPage");
async function replaceManagedBlocks2(store, localPageId, notionPageId, content) {
  return replaceManagedBlocks({
    store,
    localPageId,
    notionPageId,
    content,
...
__name(replaceManagedBlocks2, "replaceManagedBlocks");
async function importManagedBlocks2(store, page) {
  return importManagedBlocks({ hash, listAllBlockChildren, page, store });
}
__name(importManagedBlocks2, "importManagedBlocks");
...
__name(listAllBlockChildren, "listAllBlockChildren");
async function createChildPage(store, parentPageId, title2) {
  return notionRequest(store, "/pages", {
    method: "POST",
    body: {
      parent: { type: "page_id", page_id: parentPageId },
      properties: { title: [{ text: { content: title2 || "Untitled Page" } }] }
    }
  });
}
__name(createChildPage, "createChildPage");
async function createWorkspacePage(store, title2) {
  return notionRequest(store, "/pages", {
    method: "POST",
    body: {
      parent: { type: "workspace", workspace: true },
      properties: { title: [{ text: { content: title2 || "Jot" } }] }
    }
  });
}
__name(createWorkspacePage, "createWorkspacePage");
async function updatePageTitle(store, pageId, title2) {
  return notionRequest(store, `/pages/${pageId}`, {
    method: "PATCH",
    body: { properties: { title: [{ text: { content: title2 || "Untitled Page" } }] } }
  });
}
__name(updatePageTitle, "updatePageTitle");
...
__name(updateChildNotePage, "updateChildNotePage");
...
__name(updateManagedBlock, "updateManagedBlock");
...
__name(appendManagedBlocks, "appendManagedBlocks");
...
__name(appendBlocksWithFallback, "appendBlocksWithFallback");
function isTrustedOrigin(env2, origin) {
  if (!origin) return true;
  if (origin.startsWith("chrome-extension://")) return true;
  const trusted = [
    env2.WORKER_URL,
    env2.JOT_EXTENSION_ORIGIN,
    ...env2.TRUSTED_ORIGINS?.split(",") ?? []
  ].map((o) => o?.trim()).filter(Boolean);
  return trusted.includes(origin);
}
__name(isTrustedOrigin, "isTrustedOrigin");
function serverBaseUrl(env2) {
  return env2.WORKER_URL ?? "http://localhost:8787";
}
__name(serverBaseUrl, "serverBaseUrl");
...
__name(hash, "hash");
function isNotionObjectNotFound(error3) {
  return error3?.status === 404 || error3?.code === "object_not_found";
}
__name(isNotionObjectNotFound, "isNotionObjectNotFound");
function isBlockNotPageError3(error3) {
  return error3?.code === "validation_error" && /is a block, not a page|retrieve block API/i.test(error3?.message ?? "");
}
__name(isBlockNotPageError3, "isBlockNotPageError");
function updateBodyFromNotionBlock(block) {
  if (!block?.type) return {};
  return { [block.type]: block[block.type] ?? {} };
}
__name(updateBodyFromNotionBlock, "updateBodyFromNotionBlock");
...
__name(positionAfterCreatedBlocks, "positionAfterCreatedBlocks");
function mediaFallbackBlock(block) {
  const url = mediaUrlFromNotionBlock2(block);
  const isLinkable = url && !url.startsWith("data:");
  return {
    object: "block",
    type: "paragraph",
    paragraph: {
      rich_text: isLinkable ? [{ type: "text", text: { content: url, link: { url } } }] : [{ type: "text", text: { content: "[Image \u2014 not synced]" } }],
      color: "default"
    }
  };
}
__name(mediaFallbackBlock, "mediaFallbackBlock");
function mediaUrlFromNotionBlock2(block) {
  const t = block?.type;
...
__name(mediaUrlFromNotionBlock2, "mediaUrlFromNotionBlock");
function titleFromPage2(page) {
...
__name(titleFromPage2, "titleFromPage");
...
__name(isSupportedMediaMimeType, "isSupportedMediaMimeType");
...
__name(sanitizeMediaFilename, "sanitizeMediaFilename");
...
__name(mediaExtensionFromMimeType, "mediaExtensionFromMimeType");
...
// .wrangler/tmp/bundle-Iv1Ksc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Iv1Ksc/middleware-loader.entry.ts
